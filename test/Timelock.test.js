...
contract('Timelock', ([alice, bob, carol, dev, minter]) => {
    ...
    it('should also work with MasterChef', async () => {
        ...
        this.chef = await MasterChef.new(this.cake.address, this.syrup.address, dev,minter, '1000', '0', { from: alice });
        ...
        await this.chef.add('100', this.lp1.address, '100',true, { from: alice });
        await this.chef.transferOwnership(this.timelock.address, { from: alice });
        await expectRevert(
            this.chef.add('100', this.lp1.address,'100', true, { from: alice }),
